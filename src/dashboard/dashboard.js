...
const loadPlaylist  = async(endpoint,elementId) => {
    ...
    for(let {name, description,images,id} of items){
        ...
        playlistItems.innerHTML= ` <img
          src="${imageurl}" alt="${name}" class="rounded mb-2 object-contain"/>
  ...
  for(let trackItem of tracks.items.filter( item=> item.track.preview_url)){
    ...
    track.innerHTML = `    
     ...
     <section class="grid grid-cols-[auto_1fr] place-items-center gap-1">
       <img class="h-10 w-10" src="${image.url}" alt="${name}" srcset="">
...
const fillContentForPlaylist = async (playlistId)=>{
  ...
  coverElement.innerHTML =`<img class="object-contain h-36 w-36" src="${images[0].url}" alt="" srcset="">
  <section>
...
const playButton = document.querySelector("#play");
const songDurationCompleted = document.querySelector("#song-duartion-completed");
